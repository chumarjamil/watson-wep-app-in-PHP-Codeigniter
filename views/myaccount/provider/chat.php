...
<?php if($this->session->user->id && $this->session->user->id > 0) { ?>
 <script>
	...
	var chat_name = "<?php echo $user_detail->getCompany()->getCompanyName().' - '.$user->getUserName();?>"; 
	var chat_link = ""; //Similarly populate it from session for user's profile link if exists
	var chat_avatar = "https://global-patienttransfer.com/assets/images/chatserviceproviderlogo.jpg"; //Similarly populate it from session for user's avatar src if exists
	var chat_role = "provider"; //Similarly populate it from session for user's role if exists
