...
<?php if($this->session->user->id && $this->session->user->id > 0) { ?>
 <script>
	...
	var chat_name = "<?php echo $user_detail->getFirstName().' - '.$user->getEmail(); ?>"; 
	var chat_link = ""; //Similarly populate it from session for user's profile link if exists
	var chat_avatar = "https://global-patienttransfer.com/assets/images/chatpatientlogo.jpg"; //Similarly populate it from session for user's avatar src if exists
	var chat_role = "patient"; //Similarly populate it from session for user's role if exists
