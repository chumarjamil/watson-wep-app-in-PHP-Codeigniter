<?php $contact = $xref->getContact();?>
<div class="single-contact" data-xref-id="<?php echo $xref->getId();?>" data-id="<?php echo $contact->getId();?>" id="row-contact-<?php echo $xref->getId();?>">
    <div class="row pr-3">
        <div class="col-md-10 mt-2"><h4><?php echo $contact->getDisplayName();?></h4></div>
        <div class="col-md-2 pr-0" style="padding-top: .6em;">
            <div class="btn-group pt-1 pull-right" role="group">
                <button type="button" class="btn btn-secondary btn-sm" data-toggle="collapse" href="#contactDetail<?php echo $xref->getId();?>" title="See Details"><i class="fa fa-fw fa-eye"></i></button>
                <button type="button" class="btn btn-secondary btn-sm edit" data-for="hospital_contact" title="Edit Details"><i class="fa fa-fw fa-edit"></i></button>
    ...
    <div class="collapse" id="contactDetail<?php echo $xref->getId();?>">
        <div class="row">
                    ...
                    <div class="card-body address-list alternate-grid">
                        ...
                        if($addresses && count($addresses)){
                            ...
                            foreach($addresses as $address){
                                $this->load->view('myaccount/hospital/partials/display-address',['address'=>$address]);
                            }
                        }
                        ?>
                    ...
                    <?php 
                        ...
                        if($emails && count($emails)){
                            ...
                            foreach($emails as $email){
                                $this->load->view('myaccount/hospital/partials/display-email',['email'=>$email]);
                            }
                        }
                        ?>
                    ...
                    <?php 
                        ...
                        if($phone_numbers && count($phone_numbers)){
                            foreach($phone_numbers as $phone_number){
                                $class='d-none';
                                $this->load->view('myaccount/hospital/partials/display-phone',['phone_number'=>$phone_number]);
                            }
                        }
                        ?>
